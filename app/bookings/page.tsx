...
import {
  ...
  CreditCard, CheckCircle2, XCircle, BookOpen, ChevronLeft, ChevronRight,
} from 'lucide-react';
...
function statusStyle(s: string)  { return STATUS_STYLES[s]  ?? 'bg-muted text-muted-foreground'; }
function paymentStyle(s: string) { return PAYMENT_STYLES[s] ?? 'bg-muted text-muted-foreground'; }
...
function CustomerAvatar({ name }: { name: string }) {
  ...
  return (
    <div className={cn(
      'h-10 w-10 shrink-0 rounded-xl bg-gradient-to-br font-bold text-white text-sm grid place-items-center',
      colors[idx],
    )}>
      {initials(name)}
    </div>
...
export default function BookingListPage() {
  ...
  // Build the proxy URL with pagination params (proxy clamps page_size ≤ 100)
  const proxyUrl = `/api/v1/bookings?page=${page}&page_size=20`;

  const { data: rawBookings, loading, error, pagination, refetch } = useBookings<Record<string, unknown>>(
    proxyUrl,
    [],
  );
  ...
  // Unique options derived from loaded page
  ...
  const clearAll  = () => {
    setSearch(''); setBranchFilter(''); setTherapistFilter('');
    setTypeFilter(''); setStatusFilter(''); setPayFilter('');
  };
  ...
  return (
    <DashboardShell>
      <PageHeader
        title="Booking List"
        subtitle={loading ? 'Loading…' : `${filtered.length} bookings on this page`}
      />
      ...
      {!loading && filtered.length > 0 && viewMode === 'grid' && (
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((b) => (
            <div key={b.id}
              className="group rounded-2xl border border-border/50 bg-card p-4 shadow-sm transition hover:border-primary/40 hover:shadow-md animate-fade-in-up">
              ...
              <div className="flex items-center gap-3 mb-3">
                ...
                {/* Status */}
                <span className={cn('rounded-full px-2 py-0.5 text-[11px] font-semibold capitalize shrink-0', statusStyle(b.status))}>
                  {b.status}
                </span>
              </div>
              ...
              <div className="space-y-1 text-xs text-muted-foreground">
                <p className="flex items-center gap-1.5">
                  ...
                  {formatDate(b.appointment_start)}
                  <span className="ml-1">{formatTime(b.appointment_start)} – {formatTime(b.appointment_end)}</span>
                </p>
                <p className="flex items-center gap-1.5">
                  <MapPin className="h-3 w-3 shrink-0" /> {b.branch_name}
                </p>
                <p className="flex items-center gap-1.5">
                  <User className="h-3 w-3 shrink-0" /> {b.therapist_name}
                </p>
                {b.arrangement_name && (
                  <p className="flex items-center gap-1.5">
                    <Scissors className="h-3 w-3 shrink-0" /> {b.arrangement_name}
                  </p>
                )}
              </div>
              ...
              <div className="mt-3 flex items-center justify-between border-t border-border/40 pt-3">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className={cn('rounded-full px-2 py-0.5 text-[11px] font-semibold capitalize', paymentStyle(b.payment_status))}>
                    {b.payment_status || 'unpaid'}
                  </span>
                  {b.payment_gateway && (
                    <span className="text-[11px] text-muted-foreground">via {b.payment_gateway}</span>
                  )}
                </div>
                <p className="font-bold text-sm text-primary">
                  {parseFloat(b.total_amount).toFixed(3)} <span className="text-[11px] font-normal text-muted-foreground">{b.currency}</span>
                </p>
              </div>
            </div>
          ))}
                ...
                <tr key={b.id}
                  ...
                  <td className="px-4 py-3 hidden md:table-cell">
                    <span className="flex items-center gap-1 text-xs text-muted-foreground">
                      ...
                      <span className="max-w-[140px] truncate">{b.branch_name}</span>
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3 hidden sm:table-cell text-center">
                    <span className="inline-block rounded-full bg-muted/60 px-2.5 py-0.5 text-[11px] font-semibold capitalize">
                      {b.booking_type}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3 text-right">
                    <p className="font-bold text-sm text-primary whitespace-nowrap">
                      {parseFloat(b.total_amount).toFixed(3)}
                    </p>
        ...
        return (
          <div className="mt-5 flex items-center justify-between">
            <span className="text-xs text-muted-foreground">
              {pagination
                ? `Page ${page} of ${pagination.total_pages} · ${totalCount} total`
                : `Page ${page}`}
            </span>
