          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex items-center gap-3 rounded-2xl border border-border/50 bg-muted/30 px-4 py-3 min-w-0">
              ...
              <div className="min-w-0">
                <p className="text-[11px] text-muted-foreground font-medium">Service Type</p>
                <p className="text-sm font-bold truncate">Home Service</p>
              </div>
            </div>
            <div className="flex items-center gap-3 rounded-2xl border border-border/50 bg-muted/30 px-4 py-3">
              <div className={cn('grid h-9 w-9 shrink-0 place-items-center rounded-xl', cfg.pillCls)}>
...
export default function HomeServiceSchedulePage() {
  ...
  // Fetch schedule when date changes — branch is always "home"
  useEffect(() => {
    ...
    (async () => {
      ...
      setError(null);
      try {
        const url = `/api/v1/therapists/schedule/?branch_id=home&date=${selectedDate}`;
  ...
  return (
    <DashboardShell>
      ...
      <div className="mb-6 flex flex-wrap items-center gap-3">
        ...
        {/* Home Service badge (no dropdown – fixed to "home") */}
